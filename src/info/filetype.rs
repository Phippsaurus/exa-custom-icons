...
impl FileExtensions {
    ...
    fn is_compressed(&self, file: &File) -> bool {
        file.extension_is_one_of(&[
            "zip", "tar", "Z", "z", "gz", "bz2", "a", "ar", "7z", "iso", "dmg", "ez", "tc", "rar",
            "par", "tgz", "xz", "txz", "lzma", "deb", "rpm", "zst",
        ])
    }
    ...
    fn is_compiled(&self, file: &File) -> bool {
        if file.extension_is_one_of(&["beam", "class", "elc", "hi", "o", "pyc"]) {
            true
...
impl FileIcon for FileExtensions {
    fn icon_file(&self, file: &File) -> Option<char> {
        ...
        Some(match file {
            ...
            f if self.is_compressed(f) => Icons::Archive.value(),
            f if self.is_compiled(f) => Icons::Binary.value(),
            f if self.is_crypto(f) => Icons::Crypto.value(),
            f if self.is_temp(f) => Icons::Temp.value(),
            _ => return None,
        })
    }
}
