...
use output::{details, grid, lines, Mode, View};
...
impl TimeTypes {
    ...
    fn deduce(matches: &MatchedFlags) -> Result<TimeTypes, Misfire> {
        ...
        let mut fields = vec![];
        if time_types.modified {
            fields.push(PlatformMetadata::ModifiedTime);
        }
        if time_types.changed {
            fields.push(PlatformMetadata::ChangedTime);
        }
        if time_types.accessed {
            fields.push(PlatformMetadata::AccessedTime);
        }
        if time_types.created {
            fields.push(PlatformMetadata::CreatedTime);
        }
